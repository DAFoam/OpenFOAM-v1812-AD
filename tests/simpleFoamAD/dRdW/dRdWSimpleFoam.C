...
int main(int argc, char *argv[])
{
    ...
    (
        "cell",
        "152",
        "Which cell to perturb"
    );
    argList::addOption
    (
        "var",
        "U",
        "Which variable to perturb"
    );
    argList::addOption
    (   
        "eps",
    ...
    word var = word(args.optionLookup("var")());
        ...
        // AD
        {
            if(procI == myProc)
            {
                if(var=="U") U[cellI][0].setGradient(1.0);
                else if (var=="p") p[cellI].setGradient(1.0);
                else if (var=="phi") phi[cellI].setGradient(1.0);
            }
            ...
            word partDerivResName = "verify_ProcI_"+name(myProc)+"_CellI_"+name(cellI)+"_AD.txt";
        ...
        // FD
        {
            ...
            // perturb
            if (procI == myProc) {
                if(var=="U") U[cellI][0] += eps;
                else if(var=="p") p[cellI] += eps;
                else if(var=="phi") phi[cellI] += eps;
            }
            U.correctBoundaryConditions();
            p.correctBoundaryConditions();
            ...
            word partDerivResName = "verify_ProcI_"+name(myProc)+"_CellI_"+name(cellI)+"_FD.txt";
            OFstream fOut(partDerivResName);
            label glbIdx = 0;
            forAll(URes, idxJ)
            {
